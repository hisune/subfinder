...
def epilog():
    ...
    for name, subsearcher in all_subsearchers.items():
        msg += 'languages supported by {}: {}\n'.format(name, subsearcher.SUPPORT_LANGUAGES)
        msg += 'exts supported by {}: {}\n'.format(name, subsearcher.SUPPORT_EXTS)
        msg += '\n'
    return msg
...
def run(subfinder_class):
    ...
    parser.add_argument('path', help="the path of video file, or the directory contains video files")
    parser.add_argument('-l', '--languages', nargs='+', help="specify what subtitle languages you want")
    parser.add_argument('-e', '--exts', nargs='+', help="specify what subtitle formats you want")
    parser.add_argument('-m', '--method', nargs='+', type=find_method, help=method_msg())
    parser.add_argument('-k', '--keyword', help='keyworkd for the video using to search on the subtitle website')
    parser.add_argument('-c', '--conf', default='~/.subfinder.json', help='configuration file')
    parser.add_argument('--video_exts', nargs='+', help='the extensions of video file(include ".", e.g: .mp4)')
    parser.add_argument('--ignore', action='store_true', help='search subtitles even there are existing subtitles')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        '--version',
        action='version',
        version='subfinder {v}'.format(v=__version__),
        help="show subfinder's version",
    )
    parser.add_argument('--debug', action='store_true', help="print debug infomation, default to False")
    ...
    subfinder = subfinder_class(
        path=root, subsearcher_class=conf_dict['method'] if 'method' in conf_dict else None, **conf_dict
    )
    subfinder.start()
    subfinder.done()
